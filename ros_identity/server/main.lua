...
function getIdentity(source, callback)
	...
	MySQL.Async.fetchAll('SELECT identifier, firstname, lastname, dateofbirth, sex, height, job FROM `users` WHERE `identifier` = @identifier', {
...
RegisterServerEvent('unity_identity:hud')
AddEventHandler('unity_identity:hud', function()
	if(playerEntity~=nil) then
		getIdentity(playerEntity, function(data)
			if data.firstname == '' then
				TriggerClientEvent('esx_identity:identityCheck', playerEntity, false)
				TriggerClientEvent('esx_identity:showRegisterIdentity', playerEntity)
			else
				TriggerClientEvent('esx_identity:identityCheck', playerEntity, true)
				TriggerEvent('esx_identity:characterUpdated', playerEntity, data)
				TriggerClientEvent('esx_identity:sendPlayerData',playerEntity,data)
				TriggerClientEvent('esx_identity:screendown',playerEntity)
			end
		end)
	end
end)
